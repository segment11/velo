...
class RDBParserTest extends Specification {
    ...
    def 'test read entry'() {
        ...
        parser.readEntry(Unpooled.wrappedBuffer(aBytes), callback)
        then:
        1 == 1
        ...
        parser.readEntry(Unpooled.wrappedBuffer(bBytes), callback)
        then:
        1 == 1
        ...
        parser.readEntry(Unpooled.wrappedBuffer(cBytes), callback)
        then:
        1 == 1
        ...
        parser.readEntry(Unpooled.wrappedBuffer(dBytes), callback)
        then:
        1 == 1
        ...
        parser.readEntry(Unpooled.wrappedBuffer(lzfBytes), callback)
        then:
        1 == 1
        ...
        parser.readEntry(Unpooled.wrappedBuffer(quickListBytes), callback)
        then:
        ...
        parser.readEntry(Unpooled.wrappedBuffer(quickListBytes2), callback)
        then:
        callback.result instanceof RedisList
        (callback.result as RedisList).size() == 3
        ...
        parser.readEntry(Unpooled.wrappedBuffer(intSetBytes), callback)
        then:
        ...
        parser.readEntry(Unpooled.wrappedBuffer(listPackSetBytes), callback)
        then:
        callback.result instanceof RedisHashKeys
        ...
        parser.readEntry(Unpooled.wrappedBuffer(listPackHashBytes), callback)
        then:
        ...
        parser.readEntry(Unpooled.wrappedBuffer(listPackZSetBytes), callback)
        then:
    ...
    def 'test dump string'() {
        ...
        when:
        def bytesByte = RDBParser.dumpString('12'.bytes)
        parser.readEntry(Unpooled.wrappedBuffer(bytesByte), callback)
        then:
        callback.result as byte[] == '12'.bytes

        when:
        def bytesShort = RDBParser.dumpString('123'.bytes)
        parser.readEntry(Unpooled.wrappedBuffer(bytesShort), callback)
        then:
        callback.result as byte[] == '123'.bytes

        when:
        def bytesInt = RDBParser.dumpString('1234'.bytes)
        parser.readEntry(Unpooled.wrappedBuffer(bytesInt), callback)
        then:
        callback.result as byte[] == '1234'.bytes

        when:
        def bytesLong = RDBParser.dumpString('1234567890'.bytes)
        parser.readEntry(Unpooled.wrappedBuffer(bytesLong), callback)
        then:
        callback.result as byte[] == '1234567890'.bytes

        when:
        def bytes = RDBParser.dumpString('hello world'.bytes)
        parser.readEntry(Unpooled.wrappedBuffer(bytes), callback)
        then:
    ...
    def 'test dump types'() {
        ...
        rhk.add('member0')
        rhk.add('member1')
        def bytesSet = RDBParser.dumpSet(rhk)
        parser.readEntry(Unpooled.wrappedBuffer(bytesSet), callback)
        then:
        callback.result instanceof RedisHashKeys
        (callback.result as RedisHashKeys).set.size() == 2
        ...
        def bytesHash = RDBParser.dumpHash(rhh)
        parser.readEntry(Unpooled.wrappedBuffer(bytesHash), callback)
        then:
        callback.result instanceof RedisHH
        (callback.result as RedisHH).map.size() == 2
        ...
        def bytesList = RDBParser.dumpList(rl)
        parser.readEntry(Unpooled.wrappedBuffer(bytesList), callback)
        then:
        callback.result instanceof RedisList
        ...
        rz.add(0.1, 'member0')
        rz.add(0.2, 'member1')
        def bytesZSet = RDBParser.dumpZSet(rz)
        parser.readEntry(Unpooled.wrappedBuffer(bytesZSet), callback)
        then:
        callback.result instanceof RedisZSet
        (callback.result as RedisZSet).set.size() == 2
    }
