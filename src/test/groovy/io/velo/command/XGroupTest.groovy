...
class XGroupTest extends Specification {
    ...
    static final class MasterContext {
        final LocalPersist localPersist
        final OneSlot oneSlot
        final ReplPair replPairAsSlave
        final XGroup x
        final long slaveUuid

        MasterContext(LocalPersist localPersist, OneSlot oneSlot,
                      ReplPair replPairAsSlave, XGroup x, long slaveUuid) {
            this.localPersist = localPersist
            this.oneSlot = oneSlot
            this.replPairAsSlave = replPairAsSlave
            this.x = x
            this.slaveUuid = slaveUuid
        }
    }

    static final class SlaveContext {
        final LocalPersist localPersist
        final OneSlot oneSlot
        final ReplPair replPairAsMaster
        final XGroup x
        final long masterUuid

        SlaveContext(LocalPersist localPersist, OneSlot oneSlot,
                     ReplPair replPairAsMaster, XGroup x, long masterUuid) {
            this.localPersist = localPersist
            this.oneSlot = oneSlot
            this.replPairAsMaster = replPairAsMaster
            this.x = x
    ...
    private MasterContext setupMasterContext(long slaveUuid = 1L) {
        ...
        new MasterContext(localPersist, oneSlot, replPairAsSlave, x, slaveUuid)
    }
    ...
    private SlaveContext setupSlaveContext(long masterUuid = 10L) {
        ...
        new SlaveContext(localPersist, oneSlot, replPairAsMaster, x, masterUuid)
    }

    private void establishMasterReplPair(XGroup x, ReplPair replPairAsSlave, long slaveUuid) {
        ...
        x.handleRepl(mockReplRequest(replPairAsSlave, ReplType.hello, hello))
    }
    ...
    def 'test as master handle ping'() {
        given:
        def ctx = setupMasterContext()
        def localPersist = ctx.localPersist

        when:
        def ping = new Ping('localhost:6380')
        def replRequest = mockReplRequest(ctx.replPairAsSlave, ReplType.ping, ping)
        ReplReply r = ctx.x.handleRepl(replRequest) as ReplReply
        then:
        r.isReplType(ReplType.pong)

        when:
        r = ctx.x.handleRepl(replRequest) as ReplReply
        then:
        r.isReplType(ReplType.pong)

        when:
        def malformedPing = new Ping('localhost:6380:extra')
        replRequest = mockReplRequest(ctx.replPairAsSlave, ReplType.ping, malformedPing)
        r = ctx.x.handleRepl(replRequest) as ReplReply
        then:
    ...
    def 'test as master handle hello'() {
        given:
        def ctx = setupMasterContext()
        def localPersist = ctx.localPersist
        def oneSlot = ctx.oneSlot
        def replPairAsSlave = ctx.replPairAsSlave
        def x = ctx.x
        def slaveUuid = ctx.slaveUuid
        ...
        def replRequest = mockReplRequest(replPairAsSlave, ReplType.hello, hello)
    ...
    def 'test as master handle bye'() {
        ...
        def x = ctx.x
        def slaveUuid = ctx.slaveUuid
        establishMasterReplPair(x, replPairAsSlave, slaveUuid)

        when:
        def ping = new Ping('localhost:6380')
        def replRequest = mockReplRequest(replPairAsSlave, ReplType.bye, new Ping('localhost:6379:extra'))
        ReplReply r = x.handleRepl(replRequest) as ReplReply
        then:
        r.isReplType(ReplType.error)
    ...
    def 'test as master handle exists chunk segments'() {
        given:
        def ctx = setupMasterContext()
        def localPersist = ctx.localPersist
        def oneSlot = ctx.oneSlot
        ...
        def replRequest = new ReplRequest(slaveUuid, slot, ReplType.exists_chunk_segments, contentBytes, contentBytes.length)
        ReplReply r = x.handleRepl(replRequest) as ReplReply
        then:
    ...
    def 'test as master handle exists wal'() {
        given:
        def ctx = setupMasterContext()
        def localPersist = ctx.localPersist
        def replPairAsSlave = ctx.replPairAsSlave
        ...
        def requestBuffer = ByteBuffer.wrap(contentBytes)
        requestBuffer.putInt(0)
        def replRequest = new ReplRequest(slaveUuid, slot, ReplType.exists_wal, contentBytes, contentBytes.length)
        ReplReply r = x.handleRepl(replRequest) as ReplReply
        then:
    ...
    def 'test as master handle incremental big string'() {
        given:
        def ctx = setupMasterContext()
        def localPersist = ctx.localPersist
        def oneSlot = ctx.oneSlot
        ...
        def replRequest = new ReplRequest(slaveUuid, slot, ReplType.incremental_big_string, contentBytes, contentBytes.length)
        ReplReply r = x.handleRepl(replRequest) as ReplReply
        then:
    ...
    def 'test as master handle exists big string'() {
        given:
        def ctx = setupMasterContext()
        def localPersist = ctx.localPersist
        def oneSlot = ctx.oneSlot
        ...
        when:
        def contentBytes = new byte[4 + 8 * 2]
        def requestBuffer = ByteBuffer.wrap(contentBytes)
        requestBuffer.putInt(sBigString.bucketIndex())
        def replRequest = new ReplRequest(slaveUuid, slot, ReplType.exists_big_string, contentBytes, contentBytes.length)
        ReplReply r = x.handleRepl(replRequest) as ReplReply
        then:
    ...
    def 'test as master handle exists short string'() {
        given:
        def ctx = setupMasterContext()
        def localPersist = ctx.localPersist
        def replPairAsSlave = ctx.replPairAsSlave
        ...
        def requestBuffer = ByteBuffer.wrap(contentBytes)
        requestBuffer.putInt(0)
        def replRequest = new ReplRequest(slaveUuid, slot, ReplType.exists_short_string, contentBytes, contentBytes.length)
    ...
    def 'test as master handle exists dict'() {
        given:
        def ctx = setupMasterContext()
        def localPersist = ctx.localPersist
        def replPairAsSlave = ctx.replPairAsSlave
        ...
        when:
        def contentBytes = new byte[2]
        def replRequest = new ReplRequest(slaveUuid, slot, ReplType.exists_dict, contentBytes, contentBytes.length)
        ReplReply r = x.handleRepl(replRequest) as ReplReply
        then:
        r.isReplType(ReplType.error)
    ...
    def 'test as master handle exists all done'() {
        given:
        def ctx = setupMasterContext()
        def localPersist = ctx.localPersist
        def replPairAsSlave = ctx.replPairAsSlave
        ...
        when:
        def contentBytes = [0] as byte[]
        def replRequest = new ReplRequest(slaveUuid, slot, ReplType.exists_all_done, contentBytes, contentBytes.length)
        ReplReply r = x.handleRepl(replRequest) as ReplReply
        then:
    ...
    def 'test as master handle catch up'() {
        given:
        def ctx = setupMasterContext()
        def localPersist = ctx.localPersist
        def oneSlot = ctx.oneSlot
        ...
        when:
        def contentBytes = new byte[8 + 4 + 8 + 8]
        def requestBuffer = ByteBuffer.wrap(contentBytes)
        requestBuffer.putLong(oneSlot.masterUuid + 1)
        requestBuffer.putInt(0)
        requestBuffer.putLong(0)
        requestBuffer.putLong(0)
        def replRequest = new ReplRequest(slaveUuid, slot, ReplType.catch_up, contentBytes, contentBytes.length)
        ReplReply r = x.handleRepl(replRequest) as ReplReply
        then:
        r.isReplType(ReplType.error)
    ...
    def 'test as slave handle hi'() {
        ...
        cleanup:
        localPersist.cleanUp()
        if (dictMap != null) dictMap.cleanUp()
        Consts.persistDir.deleteDir()
    }

    def 'test as slave handle byeBye'() {
        given:
        def ctx = setupSlaveContext()
        def localPersist = ctx.localPersist
        def oneSlot = ctx.oneSlot
        ...
        when:
        def replRequest = mockReplRequest(Repl.test(slot, replPairAsMaster, 'test'))
        ReplReply r = ctx.x.handleRepl(replRequest) as ReplReply
        then:
        r.isEmpty()
    ...
    def 'test as slave handle s exists chunk segments'() {
        given:
        def ctx = setupSlaveContext()
        def localPersist = ctx.localPersist
        def oneSlot = ctx.oneSlot
        ...
        def replRequest = new ReplRequest(replPairAsMaster.slaveUuid, slot, ReplType.s_exists_chunk_segments, contentBytes, contentBytes.length)
        ReplReply r = x.handleRepl(replRequest) as ReplReply
        then:
    ...
    def 'test as slave handle s exists wal'() {
        given:
        def ctx = setupSlaveContext()
        def localPersist = ctx.localPersist
        def oneSlot = ctx.oneSlot
        ...
        when:
        def contentBytes = new byte[8 + 2 * Wal.ONE_GROUP_BUFFER_SIZE]
        def requestBuffer = ByteBuffer.wrap(contentBytes)
        requestBuffer.putInt(0)
        ...
        def replRequest = new ReplRequest(replPairAsMaster.slaveUuid, slot, ReplType.s_exists_wal, contentBytes, contentBytes.length)
        ReplReply r = x.handleRepl(replRequest) as ReplReply
        then:
        r.isReplType(ReplType.exists_wal)
    ...
    def 'test as slave handle s incremental big string'() {
        given:
        def ctx = setupSlaveContext()
        def localPersist = ctx.localPersist
        def oneSlot = ctx.oneSlot
        ...
        def bigStringUuid = 1L
        def bigStringKey = 'big-string'
        def contentBytes = new byte[8 + 4 + bigStringKey.length() + 1024]
        def requestBuffer = ByteBuffer.wrap(contentBytes)
        requestBuffer.putLong(bigStringUuid)
        requestBuffer.putInt(bigStringKey.length())
        requestBuffer.put(bigStringKey.getBytes())
    ...
    def 'test as slave handle s exists big string'() {
        given:
        def ctx = setupSlaveContext()
        def localPersist = ctx.localPersist
        def oneSlot = ctx.oneSlot
        ...
        when:
        def contentBytes = new byte[4 + 4 + 1]
        def requestBuffer = ByteBuffer.wrap(contentBytes)
        requestBuffer.putInt(0)
        requestBuffer.putInt(0)
        requestBuffer.put((byte) 1)
        def replRequest = new ReplRequest(replPairAsMaster.slaveUuid, slot, ReplType.s_exists_big_string, contentBytes, contentBytes.length)
        ReplReply r = x.handleRepl(replRequest) as ReplReply
        then:
    ...
    def 'test as slave handle s exists short string'() {
        given:
        def ctx = setupSlaveContext()
        def localPersist = ctx.localPersist
        def oneSlot = ctx.oneSlot
        ...
        when:
        def contentBytes = new byte[4]
        def replRequest = new ReplRequest(replPairAsMaster.slaveUuid, slot, ReplType.s_exists_short_string, contentBytes, contentBytes.length)
        ReplReply r = x.handleRepl(replRequest) as ReplReply
        then:
        r.isReplType(ReplType.exists_short_string)
    ...
    def 'test as slave handle s exists dict'() {
        given:
        def ctx = setupSlaveContext()
        def localPersist = ctx.localPersist
        def oneSlot = ctx.oneSlot
        ...
        when:
        def contentBytes = new byte[4]
        def replRequest = new ReplRequest(replPairAsMaster.slaveUuid, slot, ReplType.s_exists_dict, contentBytes, contentBytes.length)
        ReplReply r = x.handleRepl(replRequest) as ReplReply
        then:
        r.isReplType(ReplType.exists_big_string)
    ...
    def 'test as slave handle s exists all done'() {
        given:
        def ctx = setupSlaveContext()
        def localPersist = ctx.localPersist
        def oneSlot = ctx.oneSlot
        ...
        when:
        def contentBytes = [0] as byte[]
        def replRequest = new ReplRequest(replPairAsMaster.slaveUuid, slot, ReplType.s_exists_all_done, contentBytes, contentBytes.length)
        ReplReply r = x.handleRepl(replRequest) as ReplReply
        then:
        r.isReplType(ReplType.catch_up)
    ...
    def 'test as slave handle s catch up'() {
        given:
        def ctx = setupSlaveContext()
        def localPersist = ctx.localPersist
        def oneSlot = ctx.oneSlot
        ...
        def replRequest = new ReplRequest(replPairAsMaster.slaveUuid, slot, ReplType.s_catch_up, contentBytes, contentBytes.length)
        ReplReply r = x.handleRepl(replRequest) as ReplReply
        then:
        r.isEmpty()
    ...
    def 'test as slave handle pong trigger catch up'() {
        given:
        def ctx = setupSlaveContext()
        def localPersist = ctx.localPersist
        def oneSlot = ctx.oneSlot
    ...
    def 'test handle repl rejects exists short string wal group out of range'() {
        ...
        def negativeBytes = ByteBuffer.allocate(4)
                .putInt(-1)
                .array()
        ReplReply r = x.handleRepl(new ReplRequest(slaveUuid, slot, ReplType.exists_short_string, negativeBytes, negativeBytes.length)) as ReplReply
        then:
        r.isReplType(ReplType.error)
        ...
        def tooLargeBytes = ByteBuffer.allocate(4)
                ...
                .array()
        r = x.handleRepl(new ReplRequest(slaveUuid, slot, ReplType.exists_short_string, tooLargeBytes, tooLargeBytes.length)) as ReplReply
        then:
        r.isReplType(ReplType.error)
        errorMessage(r).contains('exists short string wal group index invalid')
    ...
    def 'test handle repl rejects malformed incremental big string payload'() {
        ...
        ReplReply r = x.handleRepl(new ReplRequest(replPairAsMaster.slaveUuid, slot,
                ReplType.s_incremental_big_string, tooShortBytes, tooShortBytes.length)) as ReplReply
        then:
        r.isReplType(ReplType.error)
        ...
        r = x.handleRepl(new ReplRequest(replPairAsMaster.slaveUuid, slot,
                ReplType.s_incremental_big_string, invalidKeyLengthBytes, invalidKeyLengthBytes.length)) as ReplReply
        then:
        r.isReplType(ReplType.error)
        ...
        def negativeKeyLengthBytes = ByteBuffer.allocate(8 + 4)
                ...
                .array()
        r = x.handleRepl(new ReplRequest(replPairAsMaster.slaveUuid, slot,
                ReplType.s_incremental_big_string, negativeKeyLengthBytes, negativeKeyLengthBytes.length)) as ReplReply
        then:
        r.isReplType(ReplType.error)
        errorMessage(r).contains('incremental big string key length invalid')
    ...
    def 'test handle repl rejects malformed catch up positions'() {
        ...
        def negativeFetchedFileIndexBytes = ByteBuffer.allocate(1 + 4 + 8 + 4 + 8 + 4 + segmentBytes.length)
                ...
                .array()
        ReplReply r = x.handleRepl(new ReplRequest(replPairAsMaster.slaveUuid, slot,
                ReplType.s_catch_up, negativeFetchedFileIndexBytes, negativeFetchedFileIndexBytes.length)) as ReplReply
        then:
        r.isReplType(ReplType.error)
        ...
        def negativeFetchedOffsetBytes = ByteBuffer.allocate(1 + 4 + 8 + 4 + 8 + 4 + segmentBytes.length)
                ...
                .array()
        r = x.handleRepl(new ReplRequest(replPairAsMaster.slaveUuid, slot,
                ReplType.s_catch_up, negativeFetchedOffsetBytes, negativeFetchedOffsetBytes.length)) as ReplReply
        then:
        r.isReplType(ReplType.error)
        ...
        def negativeCurrentFileIndexBytes = ByteBuffer.allocate(13)
                ...
                .array()
        r = x.handleRepl(new ReplRequest(replPairAsMaster.slaveUuid, slot,
                ReplType.s_catch_up, negativeCurrentFileIndexBytes, negativeCurrentFileIndexBytes.length)) as ReplReply
        then:
        r.isReplType(ReplType.error)
        ...
        def negativeCurrentOffsetBytes = ByteBuffer.allocate(13)
                ...
                .array()
        r = x.handleRepl(new ReplRequest(replPairAsMaster.slaveUuid, slot,
                ReplType.s_catch_up, negativeCurrentOffsetBytes, negativeCurrentOffsetBytes.length)) as ReplReply
        then:
        r.isReplType(ReplType.error)
    ...
    def 'test handle repl rejects malformed master catch up positions'() {
        ...
        ReplReply r = x.handleRepl(new ReplRequest(slaveUuid, slot, ReplType.catch_up,
                negativeFileIndexBytes, negativeFileIndexBytes.length)) as ReplReply
        then:
        r.isReplType(ReplType.error)
        ...
        r = x.handleRepl(new ReplRequest(slaveUuid, slot, ReplType.catch_up,
                negativeNeedFetchOffsetBytes, negativeNeedFetchOffsetBytes.length)) as ReplReply
        then:
        r.isReplType(ReplType.error)
        ...
        def negativeLastUpdatedOffsetBytes = ByteBuffer.allocate(8 + 4 + 8 + 8)
                ...
                .array()
        r = x.handleRepl(new ReplRequest(slaveUuid, slot, ReplType.catch_up,
                negativeLastUpdatedOffsetBytes, negativeLastUpdatedOffsetBytes.length)) as ReplReply
        then:
        r.isReplType(ReplType.error)
    ...
    def 'test finish catch up gate stays closed when big string fetches pending'() {
        given:
        def savedSlotNumber = ConfForGlobal.slotNumber
        def savedMasterSlotNumber = ConfForGlobal.masterSlotNumber
        ...
        and: 'queue a pending big-string fetch on the slave repl pair'
        replPairAsSlave.addToFetchBigStringId(999L, 0, 1L, 'big-key')
        ...
        and: '13-byte no-more-binlog response (readonly=1, fileIndex=0, offset=segmentLen)'
        ...
        when: 'finishSlaveCatchUpApply — binlog caught up but big-string fetch pending'
        method.invoke(x, slot, replPairAsSlave, oneSlot, oneSlot.metaChunkSegmentIndex, masterUuid,
                true, false, 0, 0L, 0, (long) binlogOneSegmentLength, binlogOneSegmentLength, 1)

        then: 'gate must NOT open — all slots stay unreadable'
        !localPersist.oneSlot((short) 0).canRead
        !localPersist.oneSlot((short) 3).canRead

        when: '13-byte no-more-binlog response — still pending, gate stays closed'
        ...
        when: 'pending fetch is consumed and 13-byte response arrives again'
        replPairAsSlave.doingFetchBigStringId()
        replPairAsSlave.doneFetchBigStringUuid(999L)
        sCatchUpMethod.invoke(x, slot, thirteenBytes)

        then: 'gate opens — all slots readable'
        localPersist.oneSlot((short) 0).canRead
        localPersist.oneSlot((short) 3).canRead
    ...
    def 'test hello hi reply uses binlog offset after skip apply append'() {
        ...
        def ping = new Ping('localhost:6380')
        x.handleRepl(mockReplRequest(replPairAsSlave, ReplType.ping, ping))
        ReplReply r = x.handleRepl(mockReplRequest(replPairAsSlave, ReplType.hello, new Hello(slaveUuid, 'localhost:6380'))) as ReplReply
    ...
    def 'test handle repl waits for cross slot catch up apply before advancing offsets'() {
        ...
        and:
        def targetEventloop = Eventloop.builder()
        ...
        Thread.sleep(100)
        def targetSlot = localPersist.oneSlot(slot1)
        targetSlot.setSlotWorkerEventloop(targetEventloop)
        localPersist.fixSlotThreadId(slot1, targetEventloop.eventloopThread.threadId())
        ...
        while (BaseCommand.slot(key, slotNumber).slot() != slot1) {
            key += 'x'
        }
        ...
        localPersist.cleanUp()
        Consts.persistDir.deleteDir()
        ConfForGlobal.slotNumber = slot1
    }
    ...
    def 'test hi skips exists fetch when last updated file is newer than earliest file'() {
        ...
        ReplReply r = x.handleRepl(mockReplRequest(replPairAsMaster, ReplType.hi, hi)) as ReplReply
    ...
    def 'test hi resets first slot fetched exists all done flag on master uuid change for non first slot'() {
        ...
        localPersist.fixSlotThreadId(slot, Thread.currentThread().threadId())
        localPersist.fixSlotThreadId(slot1, Thread.currentThread().threadId())
        def nonFirstSlot = localPersist.oneSlot(slot1)
        ...
        def hiReply = Repl.reply(slot1, replPairAsMaster, ReplType.hi, hi)
        def nettyBuf = Unpooled.wrappedBuffer(hiReply.buffer().array())
    ...
    def 'test handle repl waits for cross slot exists short string writes before replying'() {
        ...
        def slice = new Slice(4 + 4 + entryLength)
    ...
    def 'test handle repl waits for cross slot exists wal writes before replying'() {
        given:
        ConfForGlobal.slotNumber = slotNumber
        LocalPersistTest.prepareLocalPersist((byte) 1, slotNumber)
        def localPersist = LocalPersist.instance
        ...
        def x = new XGroup(null, null, null)
        x.from(BaseCommand.mockAGroup((byte) 0, (byte) 1, slotNumber))
        x.replPair = null

        and:
        // build a wal group payload with one short-value entry whose key hashes to target slot 1
        def walKeySlot = x.findKeyMustInSlot(slot1, 'exists-wal-cross-slot')
        def walKey = walKeySlot.rawKey()
        def cv = new CompressedValue()
        cv.seq = 42L
        cv.keyHash = walKeySlot.keyHash()
        cv.dictSeqOrSpType = 1
        cv.compressedData = new byte[10]
        def walV = new Wal.V(42L, walKeySlot.bucketIndex(), walKeySlot.keyHash(),
                CompressedValue.NO_EXPIRE, CompressedValue.NULL_DICT_SEQ, walKey, cv.encode(), false)

        def contentBytes = new byte[8 + 2 * Wal.ONE_GROUP_BUFFER_SIZE]
        def requestBuffer = ByteBuffer.wrap(contentBytes)
        requestBuffer.putInt(0)  // remote wal group index
        requestBuffer.putInt(Wal.ONE_GROUP_BUFFER_SIZE)
        // place the short-value entry in the short-value half (second half of the group buffer)
        requestBuffer.position(8 + Wal.ONE_GROUP_BUFFER_SIZE)
        requestBuffer.put(walV.encode(true))

        when:
        def reply = x.handleRepl(new ReplRequest(replPairAsMaster.slaveUuid, slot,
                ReplType.s_exists_wal, contentBytes, contentBytes.length))
        ...
        then:
        nestedReply != null
        nestedReply.isReplType(ReplType.exists_wal) || nestedReply.isReplType(ReplType.exists_all_done)
    ...
    def 'test handle repl waits for cross slot exists big string writes before replying'() {
        given:
        ConfForGlobal.slotNumber = slotNumber
        LocalPersistTest.prepareLocalPersist((byte) 1, slotNumber)
        def localPersist = LocalPersist.instance
        ...
        and:
        // find a key whose keyHash maps to target slot 1, so s_exists_big_string dispatches to the blocked eventloop
        def bigStringKeySlot = x.findKeyMustInSlot(slot1, 'exists-big-string-cross-slot')
        def bigStringUuid = 900L
        def bigStringBytes = new byte[16]
        ByteBuffer.wrap(bigStringBytes).putLong(0xCAFEBABEL).putLong(0xDEADBEEFL)

        def contentBytes = ByteBuffer.allocate(4 + 4 + 1 + 8 + 8 + 4 + bigStringBytes.length)
                .putInt(bigStringKeySlot.bucketIndex())
                .putInt(1)
                .put((byte) 1)
                .putLong(bigStringUuid)
                .putLong(bigStringKeySlot.keyHash())
        ...
        then:
        nestedReply != null
        // after bucket is done (isCurrentBucketDone=1), advance to next bucket or exists_short_string
        nestedReply.isReplType(ReplType.exists_big_string) || nestedReply.isReplType(ReplType.exists_short_string)
    ...
    def 'test handle repl rejects malformed big string payload before persisting entry'() {
        ...
        def contentBytes = ByteBuffer.allocate(4 + 4 + 1 + 8 + 8 + 4 + bigStringBytes.length + 1)
                ...
                .array()
        ReplReply r = x.handleRepl(new ReplRequest(replPairAsMaster.slaveUuid, slot,
                ReplType.s_exists_big_string, contentBytes, contentBytes.length)) as ReplReply
        then:
        r.isReplType(ReplType.error)
